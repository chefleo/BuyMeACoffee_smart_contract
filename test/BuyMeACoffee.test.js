...
describe("BuyMeACoffee Contract", function () {
  ...
  let buyMeACoffee, owner, initialOwnerBalance, tipper, tipper2, tipper3, gasSpent, receipt, totalGasSpent;
  ...
  before("deploy the contract instance first", async function () {
	...
	initialOwnerBalance = await hre.waffle.provider.getBalance(owner.address)
	...
	// console.log('GasSpent after deploy:', hre.ethers.utils.formatEther(gasSpent))
	// console.log(`totalGasSpent after deploy:`, totalGasSpent);
  ...
  it("should store a new memo when buyCoffee is called", async function () {
    ...
    await buyMeACoffee.connect(tipper).buyCoffee("Alice", "Thanks for the coffee!", tip);

    // Check that the buyCoffee function correctly stored a new memo
    assert.equal((await buyMeACoffee.getMemos()).length, 1, "Memo was not stored");
  });
  ...
  it("should send the contract balance to the owner when withdrawTips is called", async function () {
	...
	const contractBalance = await hre.waffle.provider.getBalance(buyMeACoffee.address);

    // console.log('Balance contract: ', contractBalance);
    // console.log('Balance owner:', (await getBalance(owner.address)).toString());

    // Check that the contract balance has increased
    assert.equal((await getBalance(buyMeACoffee.address)).toString(), hre.ethers.utils.formatEther(contractBalance), "Contract balance has not increased");
	...
	// console.log('GasSpent after withdrawTips:', hre.ethers.utils.formatEther(gasSpent))
	// console.log(`totalGasSpent after withdrawTips:`, hre.ethers.utils.formatEther(totalGasSpent));

    // console.log('Balance contract:', (await getBalance(buyMeACoffee.address)).toString());
    // console.log('Owner after withdraw:', (await getBalance(owner.address)).toString());

	// Check how much the owner should be have -> Initial Owner Balance - TotalGasSpent(deploy + tx) + withdrawTips
	const expectedBalance = initialOwnerBalance.sub(totalGasSpent).add(contractBalance)
	// console.log('expectedBalance:', expectedBalance)

    // Check that the contract balance has been sent to the owner
    assert.equal((await getBalance(owner.address)).toString(), hre.ethers.utils.formatEther(expectedBalance), "Contract balance was not sent to the owner");
  });

  it("should retrieve all stored memos when getMemos is called", async function () {
    // Buy some coffee using the buyCoffee function
    await buyMeACoffee.connect(tipper).buyCoffee("Alice", "Thanks for the coffee!", tip);
    await buyMeACoffee.connect(tipper2).buyCoffee("Bob", "Thanks for the coffee!", tip);
    await buyMeACoffee.connect(tipper3).buyCoffee("Charlie", "Thanks for the coffee!", tip);
    ...
    assert.equal((await buyMeACoffee.getMemos()).length, 5, "Incorrect number of memos retrieved");
  });
});
